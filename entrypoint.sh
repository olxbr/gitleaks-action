...
if [ "$GITHUB_EVENT_NAME" = "push" -o "$GITHUB_EVENT_NAME" = "workflow_dispatch" ]; then
  GITLEAKS=$($GITLEAKS_BIN --path=$GITHUB_WORKSPACE -v --report=gitleaks.json --format=json --pretty --depth=1 --leaks-exit-code=2 --redact $CONFIG)
  EXIT_CODE=$?
  ...
  cat commit_list.txt
  echo -e "\n...from now!\n"
  GITLEAKS=$($GITLEAKS_BIN --path=$GITHUB_WORKSPACE -v --report=gitleaks.json --format=json --pretty --depth=1 --leaks-exit-code=2 --redact --commits-file=commit_list.txt $CONFIG)
  EXIT_CODE=$?
fi
